...
function checkValid() {
   ...
   var menuEle = document.getElementById('productSel');
   console.log(cateEle);
   ...
   //리뷰 메뉴
      if(cateEle.value == "2000" && menuEle.value == "") {
      menu_errmsg.innerHTML = "리뷰할 상품을 선택해야 합니다."
      flag = flag && false;
      } else {
...
<!--                                        <option value="3001">상품문의</option>
                           ...
                           <th class="cate"><label>상품 선택</label></th>
                              <td class="cate">
                                 <div class="textfield">
                                    <select name="menu" id="productSel" class="productSel">
                                    <option value="">====== 선택 ======</option>
                                    <c:forEach var="rec" items="${selectMenu }">
                                      <jsp:useBean id="now" class="java.util.Date" />
						                          <fmt:formatDate value="${now}" pattern="yyyy-MM-dd" var="today" />
						                          <fmt:parseDate value="${rec.sdate}" var="sdate" pattern="yyyy-MM-dd" />
						                          <fmt:formatDate var="sdate" pattern="yyyy-MM-dd" value="${sdate}" />
                            					 <c:if test="${sdate <= today }">
                                          <option value="${rec.smenu } [${rec.sdate }]">${rec.smenu } [${rec.sdate }]</option>
                                       </c:if>
                                    </c:forEach>
                                    </select>
                                    <div>
