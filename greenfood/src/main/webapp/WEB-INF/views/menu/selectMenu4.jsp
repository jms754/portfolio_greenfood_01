	...
	function init(){
		//선택 완료버튼클릭시
			submitbtn.addEventListener("click", function(e){
				e.preventDefault();
				if(confirm("식단을 선택하시겠습니까? 선택하신 식단은 나의쇼핑정보에서 확인 가능합니다")){
					let tmp = [];
					Array.from(document.querySelectorAll('input[type="checkbox"]:checked'))
					     .forEach(function(e){
						     //console.log(e.dataset);
						     e.dataset.sid="${member.id}";
								 tmp.push(e.dataset);
								 // console.log(tmp);
					});
				  ...
				  function ajaxCall(){
					  if (this.readyState == 4 && this.status == 200) {
						  ...
						  if(this.responseText == "success"){
						  	console.log('성공!!');
						  	location.href="${pageContext.request.contextPath }/order/infoList/";
						  }else{
						  	console.log('실패!!');
						  }
					  }
				  ...
				  //post방식
				  xhttp.open("POST","http://localhost:9080/greenfood/menu/select4/${rnum}",true); 
			...
			<table class="calendar">
				...
				<tr height="180">
					...
					<c:forEach var="list" items="${list}" varStatus="status">
                        ...
                        <fmt:parseDate value="${mdate}" var="mdate" pattern="yyyy-MM-dd" />
                      <fmt:formatDate var="mdate" pattern="yyyy-MM-dd" value="${mdate}" />                         
                         
                         <c:if test="${mdate >= today }">
                        <input type="checkbox" onchange="checkBox(this)" name="sdate"  value="${mdate }" data-sdate = "${mdate }" data-smnum ="${list.mnum }" data-smenu="${list.maindish }">
                         </c:if>								<input type="hidden" name="sdate" value="${mdate }" id="day-title"/>
								 <%-- ${ day} 
