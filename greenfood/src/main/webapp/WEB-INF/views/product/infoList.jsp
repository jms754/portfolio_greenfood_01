...
#atag {background: #BCE074; padding : 5px;}
</style>
...
<form id="menufrm" method="get" >
 <table class="order_info">
        ...
        <tbody>
           ...
           <c:forEach items="${infoList }" var="info" varStatus="status">
            ...
            <tr>                           
                ...
                <c:if test="${last == status.index+1 && cnt !=1 }">
            <tr>                           
                </c:if>          
                               
            <td id="pname">${info.pname }</td>
            <td>${info.amount }</td>
            <td>${info.price }</td>
            <td>${info.state }</td>
          
            <td><button class="selectMenu2" type="submit" data-pname="${info.pname }" data-rnum="${info.rnum }">식단고르기</button></td>
        ...
        <c:if test="${Ordercnt == 0 }">
           <tr>
               <td colspan="6" style="text-align: center; height: 100px;">구매한 상품이 없습니다.</td>
            </tr>
        </c:if>
    </table> 
</form>
...
function init2(){
   ...
   var selectMenu2 = document.querySelectorAll(".selectMenu2");

   Array.from(selectMenu2).forEach(function(element){
      element.addEventListener("click",function(e){
       e.preventDefault();
       console.log(e.target);
       let menufrm = document.forms['menufrm'];
       let pname = e.target.getAttribute('data-pname');
       let rnum = e.target.getAttribute('data-rnum');
          console.log("클릭"+pname);
          
       if(pname == '그린정식 10일권' || pname == '그린정식 20일권' || pname == '그린정식 30일권'){
          console.log("클릭2"+pname.value);
          menufrm.action="${pageContext.request.contextPath }/menu/select/"+rnum;
          menufrm.submit();
       }else if(pname == '그린스페셜 10일권' || pname == '그린스페셜 20일권' || pname == '그린스페셜 30일권'){
          console.log("클릭3"+pname.value);
          menufrm.action="${pageContext.request.contextPath }/menu/select3/"+rnum;
          menufrm.submit();
       }else if(pname == '그린덮밥 10일권' || pname == '그린덮밥 20일권' || pname == '그린덮밥 30일권'){
          console.log("클릭4"+pname.value);
          menufrm.action="${pageContext.request.contextPath }/menu/select2/"+rnum;
          menufrm.submit();
       }else if(pname == '그린다이어트 10일권' || pname == '그린다이어트 20일권' || pname == '그린다이어트 30일권'){
          console.log("클릭5"+pname.value);
          menufrm.action="${pageContext.request.contextPath }/menu/select4/"+rnum;
          menufrm.submit(); 
          }
   },false);
   });
